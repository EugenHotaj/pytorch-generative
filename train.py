...
def main(args):	
  ...
  MODEL_DICT[args.model].reproduce(args.n_epochs, args.batch_size, args.log_dir, 
                                   device) 
...
if __name__ == '__main__':	
  ...
  parser.add_argument(	
      ...
      default='nade', choices=list(MODEL_DICT.keys()))	
  parser.add_argument(	
      '--n-epochs', type=int, help='number of training epochs', default=1)	
  parser.add_argument(	
      '--batch-size', type=int, help='the training and evaluation batch_size', 	
      default=128)	
  parser.add_argument(	
      '--log-dir', type=str, help='the directory where to log data',	
      default='/tmp/run')	
  parser.add_argument(
      '--use-cuda', help='whether to use CUDA', action='store_true')
